...
//对于i行相同      1. 这i行为一种颜色3种选择,再对列    3*c(n,i)*(3^(n-i) - 1)^n
...
int main(){
    init();
    cin >> n;
    ll ans = quickpow(3ll, n*n) - quickpow( quickpow(3ll, n) - 3, n);
    for(ll i = 1; i <= n; i++){
        ll now = 3*quickpow( (quickpow(3, (n-i)) - 1), n) %mod;
        now += (quickpow(3,i) - 3)*quickpow(3, (n-i)*n);
        now %= mod;
        now *= cal(n, i);
        now %= mod;
        if(i&1)
